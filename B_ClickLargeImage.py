...
def dumpsysFramestats():
    ...
    profileText = open('./output/B_ClickLargeImage/txt/AllFrame.txt', 'w')
...
def beginProcessResult():
    ...
    profileRead = open('./output/B_ClickLargeImage/txt/AllFrame.txt', 'r')
    ...
    fpsAllFrameRead = open("./output/B_ClickLargeImage/txt/FourFrame.txt", "w")
    for fpsallframe in fpsReadList:
        fpsAllFrameRead.write(fpsallframe)
    profileAllFrameRead = open("./output/B_ClickLargeImage/txt/NineFrame.txt", "w")
    ...
    for line in fpsReadList:
        ...
        recordRow += 1
    wb.save('./output/B_ClickLargeImage/excel/FourFrame.xlsx')
    ...
    dataWrite = open("./output/B_ClickLargeImage/html/FourFrame/js/data.js", "w")
    dataWrite.write(fin)
    print("FourFrameEnd")
    ...
    frameCount = 25 * e + 50
    fin = "var person_data = " + fin + ";\nvar svg_width = " + str(frameCount) + ";"
    dataWrite = open("./output/B_ClickLargeImage/html/NineFrame/js/data.js", "w")
    dataWrite.write(fin)
    ...
    for tmplist in profileDataReadList:
        ...
        recordRow += 1
    wb.save('./output/B_ClickLargeImage/excel/NineFrame.xlsx')
    print("NineFrameEnd")


def startTest():
    BeforeTestCommon()
    prepare_B_ClickLargeImage()
    reset()
    threadWillBeStart()
    beginProcessResult()
    getSystemDropRate()
...
def getSystemDropRate():
    ...
    intoGallery = "adb shell dumpsys gfxinfo com.miui.gallery framestats"
    process = subprocess.Popen(intoGallery, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    profileText = open('./output/B_ClickLargeImage/txt/SystemDumpAll.txt', 'w')
    while True:
        nextline = process.stdout.readline()
...
def getFinalResult():
    ...
    intoGallery = "adb shell dumpsys gfxinfo com.miui.gallery framestats"
    process = subprocess.Popen(intoGallery, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    profileText = open('./output/B_ClickLargeImage/FinalResult.txt', 'w')
    while True:
        nextline = process.stdout.readline()
        nextlineDecode = nextline.decode()
        if nextlineDecode == "" or nextlineDecode is None:
