...
var all_charts = ['Vsync_IntendedVsync', 'HandleInputStart_Vsync', 'AnimationStart_HandleInputStart', 'PerformTraversalsStart_AnimationStart',
        'DrawStart_PerformTraversalsStart', 'SyncQueued_DrawStart', 'SyncStart_SyncQueued', 'IssueDrawCommandsStart_SyncStart', 'FrameCompleted_IssueDrawCommandsStart'],
    all_chart_titles = ['Vsync_IntendedVsync', 'HandleInputStart_Vsync', 'AnimationStart_HandleInputStart', 'PerformTraversalsStart_AnimationStart',
        'DrawStart_PerformTraversalsStart', 'SyncQueued_DrawStart', 'SyncStart_SyncQueued', 'IssueDrawCommandsStart_SyncStart', 'FrameCompleted_IssueDrawCommandsStart'];

var have = {
    Vsync_IntendedVsync: false,
    HandleInputStart_Vsync: false,
    AnimationStart_HandleInputStart: false,
    PerformTraversalsStart_AnimationStart: false,
    DrawStart_PerformTraversalsStart: false,
    SyncQueued_DrawStart: false,
    SyncStart_SyncQueued: false,
    IssueDrawCommandsStart_SyncStart: false,
    FrameCompleted_IssueDrawCommandsStart: false,
    unknown: false
};
...
for (var i = 0; i < person_data_keys.length; i++) {
    ...
    if (year != unknownYear) {
        ...
        person_data_without_unknown.push({
            'year': year,
            'Vsync_IntendedVsync': counts.Vsync_IntendedVsync,
            'HandleInputStart_Vsync': counts.HandleInputStart_Vsync,
            'AnimationStart_HandleInputStart': counts.AnimationStart_HandleInputStart,
            'PerformTraversalsStart_AnimationStart': counts.PerformTraversalsStart_AnimationStart,
            'DrawStart_PerformTraversalsStart': counts.DrawStart_PerformTraversalsStart,
            'SyncQueued_DrawStart': counts.SyncQueued_DrawStart,
            'SyncStart_SyncQueued': counts.SyncStart_SyncQueued,
            'IssueDrawCommandsStart_SyncStart': counts.IssueDrawCommandsStart_SyncStart,
            'FrameCompleted_IssueDrawCommandsStart': counts.FrameCompleted_IssueDrawCommandsStart
        });
    }
}
...
function getMax(person_data, includeUnknown) {
    ...
    return d3.max(person_data, function (d) {
        ...
        if (bars_are == "stacked") {
            return d.Vsync_IntendedVsync + d.HandleInputStart_Vsync + d.AnimationStart_HandleInputStart + d.PerformTraversalsStart_AnimationStart + d.DrawStart_PerformTraversalsStart + d.SyncQueued_DrawStart + d.SyncStart_SyncQueued + d.IssueDrawCommandsStart_SyncStart + d.FrameCompleted_IssueDrawCommandsStart;
        }

        return d3.max([d.Vsync_IntendedVsync, d.HandleInputStart_Vsync, d.AnimationStart_HandleInputStart, d.PerformTraversalsStart_AnimationStart, d.DrawStart_PerformTraversalsStart, d.SyncQueued_DrawStart, d.SyncStart_SyncQueued, d.IssueDrawCommandsStart_SyncStart, d.FrameCompleted_IssueDrawCommandsStart]);
    });
...
function getYAxisTickNumber(max_value, bars_are) {
    ...
    else if (bars_are != 'seperate' && max_value >= y_ticks * 9) {
        y_ticks *= 9;
    }
    return y_ticks;
}
...
for (var i = 0; i < person_data.length; i++) {
    person_data_dummy.push({
        ...
        'Vsync_IntendedVsync': 0,
        'HandleInputStart_Vsync': 0,
        'AnimationStart_HandleInputStart': 0,
        'PerformTraversalsStart_AnimationStart': 0,
        'DrawStart_PerformTraversalsStart': 0,
        'SyncQueued_DrawStart': 0,
        'SyncStart_SyncQueued': 0,
        'IssueDrawCommandsStart_SyncStart': 0,
        'FrameCompleted_IssueDrawCommandsStart': 0
    });
}
...
for (var i = 0; i < charts.length; i++) {
    ...
    if (have[chart]) {
        ...
        var xScale = d3.scale.ordinal()
                .domain(xScaleDomain)
                .rangeRoundBands([0, chart_width], 0)
            ;
...
function updateCharts(duration, delay) {
    ...
    if (bars_are == "stacked" || bars_are == "split") {
        ...
        var title = "";
        if (charts.length == 9)
            title = chart_titles[0] + ", " + chart_titles[1] + ", " + chart_titles[2] + ", " + chart_titles[3] + ", " + chart_titles[4] + ", " + chart_titles[5] + ", " + chart_titles[6]+ ", " + chart_titles[7] + ", " + chart_titles[8] + " and " + chart_titles[9];
            ...
            bars.transition().duration(duration).delay(delay)
                .attr("x", function (d, i) {
                    ...
                    if (bars_are == "split") {
                        ...
                        else if (chart == getChart(3))
                            offset = xScale.rangeBand() * 3 / charts.length;
                        else if (chart == getChart(4))
                            offset = xScale.rangeBand() * 4 / charts.length;
                        else if (chart == getChart(5))
                            offset = xScale.rangeBand() * 5 / charts.length;
                        else if (chart == getChart(6))
                            offset = xScale.rangeBand() * 6 / charts.length;
                        else if (chart == getChart(7))
                            offset = xScale.rangeBand() * 7 / charts.length;
                        else if (chart == getChart(8))
                            offset = xScale.rangeBand() * 8 / charts.length;
                        else if (chart == getChart(9))
                            offset = xScale.rangeBand() * 9 / charts.length;
                ...
                .attr("y", function (d) {
                    ...
                    if (bars_are == "stacked") {
                        if (chart == getChart(0))
                            ...
                            for (j = 3; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(3))
                            for (j = 4; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(4))
                            for (j = 5; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(5))
                            for (j = 6; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(6))
                            for (j = 7; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(7))
                            for (j = 8; j < charts.length; j++)
                                value += d[charts[j]];
                        else if (chart == getChart(8))
                            for (j = 9; j < charts.length; j++)
                                value += d[charts[j]];
                    }
                ...
                .attr("height", function (d) {
                    var h=chart_height - yScale(d[chart]);
                    if(h<0)
                     console.log(d);
